...
function SuperAdminWindow(args) {'use strict';
    ...
    function populateShopDB(e) {
        ...
        user.geolocalize(function() {
            ...
            var DataManager = require("/services/DataManager"), dm = new DataManager();
            dm.silent = true;
            dm.doCall('GET', GmapsReq, Tools.Hash2Qparams(GmapsOptions), null, function(json) {
                ...
                for ( i = 0; i < results.length; i++) {
                    ...
                    if (elem.hasOwnProperty('geometry')) {
                        ...
                        newShop.ident = elem.id;
                        // uniqueness !! GREAT
                        newShop.points = {
                            stepin : Math.round(2 + Math.random(5)) * 50,
                            stepout : Math.round(2 + Math.random(5)) * 50
                        };
                        newShop.create();
                    }
                }
    ...
    var data = [{
        title : "Ultradata is running",
        hasCheck : Ti.App.Properties.getBool('isUDRunning')
    },
    ...
    //        { title : "Stop Ultradata", hasChild :false, action : function(e) { ApplicationTabGroup.stopUD(); } },
    {
        ...
        title : "Clear all user invitations",
        hasChild : false,
        action : clearAllInvitations
    }, {
        title : "Reset first launch",
        hasChild : false,
        action : resetFirstLaunch
    }, {
        title : "List users",
        hasChild : true,
        req : "/ui/admin/ListUsersWindow"
    }, {
        title : "List shops",
        hasChild : true,
        req : "/ui/admin/ListShopsWindow"
    }, {
        title : "Clean file cache",
        hasChild : false,
        action : cleanFileCache
    }, {
        ...
        title : "Show log file",
        hasChild : true,
        req : "/ui/admin/ShowLogWindow"
    }, {
        title : "Populate shop DB",
        hasChild : false,
        action : populateShopDB
    }];
