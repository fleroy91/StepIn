...
function Shop(json) {'use strict';
    ...
    // For the rayon not STEP IN Entrée

    this.getPointsRayon = function(rayon) {
        // return this.RayonPoints;
        return rayonArray[rayon].points;
    };
    ...
    this.retrieveArticles = function(func) {
        var Article = require('model/Article'), art = new Article();
        var qparams = {
            'shop.url' : this.getUrl()
        };
    ...
    this.getExtraFormWindowOptions = function(crud) {
        return ((crud === 'read') ? {
            addStepsActions : true
        } : {
            addLocalizationButton : true
        });
    };
    ...
    this.getRayonName = function(rayon) {
        return rayonArray[rayon].name;
    };

    this.getPoints = function(action_kind, rayon) {
    ...
    this.retrieveScansOfCatalog = function(cindex, func, rewards, finalFunc) {
        ...
        if (cindex < this.catalogs.length) {
            ...
            this.getList(scan, Tools.Hash2Qparams({
                "catalog.url" : catalog.getUrl()
            }), _addNewScans(this));
        } else {
            ...
            return function(e) {
                if (e.annotation && e.clicksource === 'pin') {
                    if (e.map.selectedAnnotation && e.map.selectedAnnotation !== e.annotation) {
                        e.map.selectedAnnotation.setImage(e.map.selectedAnnotation.imgNormal);
                        //'/images/annotation-stepin.png');
                    }
                ...
                if (e.clicksource === "rightButton" || e.clicksource === "rightView") {
                    var ShopDetailWindow = require('ui/common/ShopDetailWindow'), win = new ShopDetailWindow(self, tabGroup);
                    tabGroup.openWindow(null, win, {
                        animated : true
                    });
                }
            };
    ...
    this.addOverHeader = function(view, tabGroup, isBig, gotoShop) {
        ...
        function showMap() {
            var MapDetailWindow = require('/ui/common/MapDetailWindow'), swin = new MapDetailWindow(shop);
            tabGroup.activeTab.open(swin, {
                animated : true
            });
        }
    ...
    this.createTableRow = function(tabGroup) {
        ...
        function gotoShop(self, toggle) {
            return function() {
                if (toggle) {
                    var ShopDetailWindow = require("ui/common/ShopDetailWindow"), swin = new ShopDetailWindow(self, tabGroup);

                    tabGroup.openWindow(null, swin, {
                        animated : true
                    });
                } else {
                    var ShopDetailStepInWindow = require("ui/common/ShopDetailStepInWindow"), stepWin = new ShopDetailStepInWindow(self, tabGroup);

                    tabGroup.openWindow(null, stepWin, {
                        animated : true
                    });
                }
            };
        ...
        var stepCataView = createButton(' ' + (nbCatalogs === 0 ? 50 : this.catalogPoints * nbCatalogs) + ' steps', '/images/catalog.png', '50%');
        stepCataView.left = null;
        stepCataView.right = 0;
        internView.add(stepCataView);

        stepInView.addEventListener('click', gotoShop(this, false));
        stepCataView.addEventListener('click', gotoShop(this, true));
        ntop += buttonHeight;
